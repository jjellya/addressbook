...
create table `person_info`(
	...
	`category_id` int not null default 1 comment '所属组',
...
alter table person_category add foreign key (user_id) references user_info(user_id);
alter table person_info add foreign key (category_id) references person_category(category_id);
